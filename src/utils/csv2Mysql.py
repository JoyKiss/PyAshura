...
#从创建数据库连接
def get_conn(database='test'):
    conn = pymysql.connect(host='localhost', port=3306, user='root', passwd='root', db=database, charset='utf8')
    return conn
...
def readCsvToMysqlNoHead(filename,tableName,database='test',dropTable='1',delimiter='\t'):
    with codecs.open(filename=filename, mode='r', encoding='utf-8') as f:
      ...
      for index,item in enumerate(reader):
        ...
        if item[0] is None or item[0] == '':  # item[1]作为唯一键，不能为null
              continue
        sql="insert into "+ tableName + " values(%s)" % (','.join('"' + str(c).replace("\\", "\\\\").replace("\"", "\\\"").replace("\'", "\\\'") + '"' for c in item))
        print sql
        cur.execute(sql)
      ...
      cur.close()
      conn.close()
def readCsvToMysqlHead(filename,tableName,database='test',dropTable='1',delimiter='\t'):
    with codecs.open(filename=filename, mode='r', encoding='utf-8') as f:
      reader = csv.reader(f,delimiter = delimiter)
      conn = get_conn(database)
      cur = conn.cursor()
      ...
      for index,item in enumerate(reader):
        if index == 0:
          tableCreate(item,tableName,cur,'0')
        else:
            if item[0] is None or item[0] == '':  # item[1]作为唯一键，不能为null
                  continue
            sql="insert into "+ tableName + " values(%s)" % (','.join('"' + str(c).replace("\\", "\\\\").replace("\"", "\\\"").replace("\'", "\\\'") + '"' for c in item))
...
def tableCreate(heads,tableName,cur,headFlag='1'):
  tableCheckSql = "DROP TABLE IF EXISTS `"+tableName+"`;"
  cur.execute(tableCheckSql)
  creatsql = "CREATE TABLE `"+tableName+"` ("
  print len(heads)
  ...
  for index in range(len(heads)):
    ...
    if index == len(heads) -1 :
      col = col+"`"+head.encode('utf-8').decode('utf-8-sig')+"` text DEFAULT NULL"
    else:
      col = col+"`"+head.encode('utf-8').decode('utf-8-sig')+"` text DEFAULT NULL,"
  creatsql = creatsql + col + ") ENGINE=InnoDB DEFAULT CHARSET=utf8;"
  creatsql =  creatsql.encode('utf-8').decode('utf-8-sig')
...
if __name__ == '__main__':
    ...
    readCsvToMysqlNoHead(unicode('./test.csv'),"b",'test',1,',')